...
async function CrearBaseDatosNoExistente() {
    await db.open('./.data/pymes.db')
    let existe = false;
    let res = null;
  
    // Cuenta la cantidad de tablas con el nombre 'usuarios'
    ...
    // Cuenta la cantidad de tablas con el nombre 'articulos'
    existe = false;
    res = await db.get("SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name= 'articulosfamilias'", []);
    // si la cantidad de tablas es mayor a 0, entonces la tabla existe
    if (res.contar > 0) existe = true;
    // si la tabla no existe, la creamos
    if (!existe) {
...
CrearBaseDatosNoExistente();

module.exports = CrearBaseDatosNoExistente;